...
const server = setupServer(
  rest.get('http://localhost:5000/api/bugs', (req, res, ctx) => {
    return res(ctx.status(200), ctx.json({ data: mockBugs }));
  }),
  rest.post('http://localhost:5000/api/bugs', (req, res, ctx) => {
    const newBug = { ...req.body, _id: (mockBugs.length + 1).toString() };
    mockBugs = [newBug, ...mockBugs];
    return res(ctx.status(201), ctx.json({ data: newBug }));
  }),
  rest.put('http://localhost:5000/api/bugs/:id', (req, res, ctx) => {
    ...
    if (index !== -1) {
      ...
      return res(ctx.status(200), ctx.json({ data: mockBugs[index] }));
    }
